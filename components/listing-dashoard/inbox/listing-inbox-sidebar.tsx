...
export function ListingInboxSidebar({
  listingId,
  listingMail,
  data,
  onMailClick,
  ...props
}: React.ComponentProps<typeof Sidebar> & ListingInboxSidebarProps) {
  ...
  const filteredMails = React.useCallback(() => {
    ...
    if (activeItem.title.toLowerCase() === "sent") {
      filtered = filtered.filter((mail) => mail.from === listingMail);
    } else if (activeItem.title.toLowerCase() === "archive") {
      filtered = filtered.filter(
        (mail) => mail.archived === true && mail.to === listingMail
      );
    } else if (activeItem.title.toLowerCase() === "trash") {
      filtered = filtered.filter(
        (mail) => mail.trashed === true && mail.to === listingMail
      );
    } else {
      filtered = filtered.filter(
        (mail) => !mail.archived && !mail.trashed && mail.to === listingMail
      );
    }
    ...
    if (readMails && activeItem.title.toLowerCase() !== "sent") {
      filtered = filtered.filter(
        (mail) => !mail.read && mail.to === listingMail
      );
    }
