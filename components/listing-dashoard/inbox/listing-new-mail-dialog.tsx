...
const formSchema = z.object({
  intendedReceiver: z.union([z.literal("exploreinn"), z.literal("user")]),
...
export const NewListingMailDialogForm = () => {
  ...
  const onSubmit = async (formData: FormSchema): Promise<void> => {
    ...
    try {
      ...
      const res = await sendMailfromListing({
      ...
      <DialogContent className="sm:max-w-[630px]">
        <ScrollArea className="h-[650px] pe-3">
          <DialogHeader>
            ...
            <DialogDescription>
              Send a new mail to either one of your guest or to the exploreinn
              support team.
            </DialogDescription>
          </DialogHeader>
                      ...
                      <FormControl>
                        ...
                        >
                          ...
                          <FormItem className="flex items-center space-x-3 space-y-0">
                            ...
                            <FormLabel className="font-medium">
                              Guest with a booking at this listing
                            </FormLabel>
                          </FormItem>
                        </RadioGroup>
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="w-max">
                      ...
                      <Select
                        onValueChange={(value) => {
                          const [listingId, bookingId, guestEmail] =
                            value.split(" ");
                          field.onChange(bookingId);
                          form.setValue("listingId", listingId);

                          if (form.getValues("intendedReceiver") === "user") {
