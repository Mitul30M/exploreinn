...
export function AdminInboxSidebar({
  data,
  onMailClick,
  ...props
  ...
  const filteredMails = React.useCallback(() => {
    ...
    if (activeItem.title.toLowerCase() === "sent") {
      filtered = filtered.filter(
        (mail) => mail.from === exploreinnSupportEmail
      );
    } else if (activeItem.title.toLowerCase() === "archive") {
      filtered = filtered.filter(
        (mail) => mail.archived === true && mail.to === exploreinnSupportEmail
      );
    } else if (activeItem.title.toLowerCase() === "trash") {
      filtered = filtered.filter(
        (mail) => mail.trashed === true && mail.to === exploreinnSupportEmail
      );
    } else {
      filtered = filtered.filter(
        (mail) =>
          !mail.archived && !mail.trashed && mail.to === exploreinnSupportEmail
      );
    }
    ...
    if (readMails && activeItem.title.toLowerCase() !== "sent") {
      filtered = filtered.filter(
        (mail) => !mail.read && mail.to === exploreinnSupportEmail
      );
    }
  ...
  const handleMailClick = (mail: TMails) => {
    ...
    toast({
      title: `${mail.subject}`,
      description: `from: ${mail.sender?.email ?? mail.from}, ${format(
